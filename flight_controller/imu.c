...
uint8_t imu_initialize_hardware(imuSelector_et imu_selector, gyroData_st *gyro_offsets, accelData_st *accel_offsets)
{
	switch(imu_selector){
		...
		case L3GD_NORMAL_AND_LSM303DLHC_DMA_TIM_INT: {
			...
			tim10_delay_ms(1000);
			l3gd20_compute_gyro_offsets(gyro_offsets);
			if(lsm303_init_dma_with_timing() != 0) return 1; 
			...
			if(lsm303_verify_accel_offset_integrity(*accel_offsets) == 1) return 1; 
		} break;
...
uint8_t imu_fetch_filtered_gyro_data(imuSelector_et gyro_selector, imuDataCollection_st *imu_data)
{
	switch(gyro_selector){
		...
		case MPU6050_NORMAL: {
			if(mpu6050_gyro_raw_to_structs(imu_data->dataBuffer, &(imu_data->gyroDataRaw)) != 0) return 1;
		} break; 
		
		case L3GD20_NORMAL: {
			if(l3gd20_read_gyro(&(imu_data->gyroDataRaw)) != 0) return 1;
		} break;
	...
	switch(imu_selector){
		...
		case MPU6050_NORMAL:
			if(mpu6050_imu_raw_to_structs(imu_data->dataBuffer,
									   ...
									   &(imu_data->gyroDataRaw)) != 0) return 1;
		 break; 
		...
		case L3GD_NORMAL_AND_LSM303DLHC_NORMAL: {
			...
			if(lsm303_imu_raw_to_structs(imu_data->dataBuffer,
										  &(imu_data->accelDataRaw)) != 0) return 1;
		} break;
							...
							accelData_st accel_filtered)
{
	...
	kalman_variables_y->intermediateAngle = atan(-accel_filtered.roll/ \
	  	sqrt(accel_filtered.pitch * accel_filtered.pitch + accel_filtered.z * accel_filtered.z)) * RAD_TO_DEG;

	kalman_variables_x->intermediateAngle  = atan(accel_filtered.pitch / \
		sqrt(accel_filtered.roll * accel_filtered.roll+ accel_filtered.z * accel_filtered.z)) * RAD_TO_DEG;
						   ...
						   float dt)
{
	...
	if ((kalman_variables_x->intermediateAngle < -90 && kalman_variables_x->finalAngle > 90) || \
		...
		if (fabs(kalman_variables_x->finalAngle) > 90) kalman_variables_y->gyroRate = -kalman_variables_y->gyroRate; 
...
{
	if (!mahoney_vars->anglesComputed) compute_angles(mahoney_vars);
