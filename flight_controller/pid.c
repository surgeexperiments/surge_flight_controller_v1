...
#define CALCULATE_PID_STABILIZATION(PID, ACTUAL_ANGLE, DESIRED_ANGLE)							\
	...
	if(PID.integral > PID.pidMaxOutput) PID.integral = PID.pidMaxOutput;						\
	else if (PID.integral < -PID.pidMaxOutput) PID.integral = -PID.pidMaxOutput;				\
	PID.output = PID.proportional + PID.integral + PID.derivative;								\
	if(PID.output > PID.pidMaxOutput) PID.output = PID.pidMaxOutput;							\
	else if (PID.output < -PID.pidMaxOutput) PID.output = -PID.pidMaxOutput;					\
...
{
	roll->proportional = 0; roll->integral = 0; roll->derivative = 0;
	roll->previousPidError = 0; roll->output = 0; 
	...
	pitch->proportional = 0; pitch->integral = 0; pitch->derivative = 0;
	pitch->previousPidError = 0; pitch->output = 0;
	...
	yaw->proportional = 0; yaw->integral = 0; yaw->derivative = 0;
	yaw->previousPidError = 0; yaw->output = 0;
	...
	roll_angle->proportional = 0; roll_angle->integral = 0;
	roll_angle->derivative = 0; roll_angle->previousPidError = 0; roll_angle->output = 0;
	...
	pitch_angle->proportional = 0; pitch_angle->integral = 0; pitch_angle->derivative = 0;
	pitch_angle->previousPidError = 0; pitch_angle->output = 0;
