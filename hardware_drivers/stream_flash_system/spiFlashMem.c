...
uint8_t waitUntilChipIsReadyWithTimeout(uint16_t thresholdTime)
{
    ...
    while (1)
    {
        if (isChipBusy() == 0)
            break;
        if ((millis_passed() - initTime) > thresholdTime)
            return 0;
    }
    return 1;
}
...
uint32_t fastReadBytes(uint32_t address, uint8_t *buffer, uint32_t numberOfBytesToRead)
{
    ...
    if ((address < 0) || (address > LASTFLASH_MEMADDR) || (numberOfBytesToRead < 1))
        return 0;
     ...
     */
    if (waitUntilChipIsReadyWithTimeout(500) == 0)
        return 0;
...
uint32_t readBytes(uint32_t address, uint8_t *buffer, uint32_t numberOfBytesToRead)
{
    ...
    if (waitUntilChipIsReadyWithTimeout(10) == 0)
        return 0;
...
uint32_t writeBytes(uint32_t address, uint8_t *myString, uint32_t stringLength)
{
    ...
    if ((address < 0) || (address > LASTFLASH_MEMADDR) || (stringLength < 1))
        return 0;
...
uint32_t writeBytesForStreamWrite(uint32_t address, uint8_t *myString, uint32_t stringLength, uint16_t *dim1Byte, uint16_t *dim2Byte)
{
     ...
     */
    if ((address < 0) || (address > STREAMWRITE_LASTMEMADDR) || (stringLength < 1))
        return 0;
    ...
    while (stringLength > bytesWritten)
    {
        ...
        if (waitUntilChipIsReadyWithTimeout(5) == 0)
            return bytesWritten;
...
uint8_t eraseSector(uint32_t address)
{
    ...
    if (waitUntilChipIsReadyWithTimeout(410) == 0)
        return 0;
    return 1;
...
uint8_t erase32KBBlock(uint32_t address)
{
    ...
    if (waitUntilChipIsReadyWithTimeout(1210) == 0)
        return 0;
...
uint8_t erase64KBBlock(uint32_t address)
{
    ...
    if (waitUntilChipIsReadyWithTimeout(2010) == 0)
        return 0;
