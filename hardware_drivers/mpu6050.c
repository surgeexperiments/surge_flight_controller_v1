...
static uint8_t single_byte_write_sequence(uint8_t internal_register, uint8_t command){
	...
	if(i2c_start(I2C1, MPU6050_WRITE, 0) != 0) return 1;		
	
	/* RA + receive ACK,  i2c_write ret error if ack not received */
	if(i2c_write_byte(I2C1, internal_register) != 0) return 2;
	
	/* DATA + receive ACK, ev check for error (ack not received) */
	if(i2c_write_byte(I2C1, command) != 0) return 3; 	
	
	/* Send P (stop condition) */
	if(i2c_stop(I2C1) != 0) return 4; 
...
static uint8_t setup_no_dma(void){
	...
	if(single_byte_write_sequence(MPU6050_REG_PWR_MGMT_1, 0x00) != 0) return 1; 
	
	/* Set gyro for 500dps in: GYRO_CONFIG register */
	if(single_byte_write_sequence(MPU6050_REG_GYRO_CONFIG, 0x08) != 0) return 2;
	/* Set accel for : +-8g (af_sel == 2) */
	if(single_byte_write_sequence(MPU6050_REG_ACCEL_CONFIG, 0x10) != 0) return 3; 
	/* Set digital low-pass filter to 43hz */
	if(single_byte_write_sequence(MPU6050_REG_CONFIG, 0x03) != 0) return 4; 
...
static uint8_t setup_with_interrupt(void){
	...
	// Reset the MPU
	if(single_byte_write_sequence(MPU6050_REG_PWR_MGMT_1, 1U<<7) != 0) return 1; 
	...
	*/
	...
	if(single_byte_write_sequence(MPU6050_REG_PWR_MGMT_1, 0x3) != 0) return 1; 

	if(single_byte_write_sequence(SMPLRT_DIV, 0x01) != 0) return 3; 
	
	/* Set gyro for 500dps in: GYRO_CONFIG register */
	if(single_byte_write_sequence(MPU6050_REG_GYRO_CONFIG, 0x08) != 0) return 4;
	
	/* Set accel for : +-8g (af_sel == 2) */
	if(single_byte_write_sequence(MPU6050_REG_ACCEL_CONFIG, 0x10) != 0) return 5; 
	
	/* Set digital low-pass filter to 43hz (check if this is effective!) */
	if(single_byte_write_sequence(MPU6050_REG_CONFIG, 0x03) != 0) return 6; 
	
	/* reset gyro and accel sensor */ 
	if(single_byte_write_sequence(SIGNAL_PATH_RESET, 0x07) != 0) return 7;
	...
	*/
	...
	if(single_byte_write_sequence(INT_PIN_CFG , 0x1<<4) != 0) return 9; 
	if(single_byte_write_sequence(INT_ENABLE, 0x1) != 0) return 10; 
...
uint8_t mpu6050_get_device_id(void){
	uint8_t devId;
	if(single_byte_read_sequence(MPU6050_REG_WHOAMI, &devId) != 0) return 0;
	return devId;
}
...
{
	if(burst_read_sequence(MPU6050_REG_GYRO, data_buffer, 6) != 0) return 1; 
	return 0; 
}
...
{
	if(burst_read_sequence(MPU6050_REG_ACCEL, data_buffer, 14) != 0) return 1;
	return 0; 
}
...
uint8_t mpu6050_gyro_raw_to_structs(uint8_t *mpu6050_raw_output, gyroData_st *gyro_output) 
{
	...
	if(fetch_gyro_xyz(mpu6050_raw_output) != 0) return 1; 
...
uint8_t mpu6050_compute_imu_offsets(gyroData_st *gyro_offsets, accelData_st *accel_offsets){
	...
	/* Sanity check */
	gyro_offsets->pitch = 0; gyro_offsets->roll = 0; gyro_offsets->yaw = 0;
	accel_offsets->pitch = 0; accel_offsets->roll = 0; accel_offsets->z = 0; 
