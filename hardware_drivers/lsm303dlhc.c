...
static uint8_t lsm303_init(void)
{
	...
	if(lsm303_single_byte_write_sequence(LSM303_REGISTER_ACCEL_CTRL_REG1_A, 0x97) != 0) return 1; 
	/* Init to +-8g */
	if(lsm303_single_byte_write_sequence(LSM303_REGISTER_ACCEL_CTRL_REG4_A, (1U <<5) | (1U << 3)) != 0) return 1; 
	return 0; 
}
...
static uint8_t lsm303_init_dma(void)
{
	...
	if(lsm303_single_byte_write_sequence(LSM303_REGISTER_ACCEL_CTRL_REG3_A, (1U<<4)) != 0) return 1; 
	
	/* Init to +-8g */
	if(lsm303_single_byte_write_sequence(LSM303_REGISTER_ACCEL_CTRL_REG4_A, (1U<<5)) != 0) return 1; 
	
	/* Latch interrupt request on INT1_SRC register, with INT1_SRC register cleared by reading INT1_SRC itself */
	if(lsm303_single_byte_write_sequence(LSM303_REGISTER_ACCEL_CTRL_REG5_A, (1U<<2)) != 0) return 1; 
...
{
	uint8_t myData;
	if(lsm303_byte_read_sequence(LSM303_REGISTER_ACCEL_INT1_SOURCE_A, &myData) != 0) return 1;
	return 0;
}
...
{
	if(lsm303_data_read(data_buffer, 6) != 0) return 1; 
...
uint8_t lsm303_single_byte_write_sequence(uint8_t internal_register, uint8_t command)
{
	...
	if(i2c_start(I2C1, LSM303_ADDRESS_ACCEL_WRITE, 0) != 0) return 1;	
